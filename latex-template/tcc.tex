...
\begin{document}
  ...
  \begin{abstract}
    This article analyzes the theoretical foundations and importance of backpressure strategies—such as flow control, buffering, rate limiting, circuit breakers, and load shedding—within the context of Apache Kafka in distributed systems. The study focuses on an experimental evaluation of Apache Kafka, exploring how its native architecture manages throughput and scalability. Through practical experiments, the article assesses the impact of key features—including throttling/quotas, buffers, the pull-based pub/sub model, and consumer groups—on maintaining a continuous data flow.
  \end{abstract}

  \begin{resumo}
    Este artigo analisa os fundamentos teóricos e a importância de estratégias de backpressure — como controle de fluxo, buffers, rate limiting, circuit breaker e load shedding com Apache Kafka em sistemas distribuidos. O estudo tem como foco central a avaliação experimental do Apache Kafka, explorando, como sua arquitetura nativa atua no gerenciamento de vazão e escalabilidade. Por meio de experimentos práticos com a ferramenta, são avaliados os impactos de recursos essenciais como throttling/quotas, buffers, modelo pub/sub baseado em pull e consumer groups na sustentação do fluxo contínuo de dados.
  \end{resumo}
  ...
  \begin{itemize}
    ...
    \item \textbf{Brokers}: São servidores responsáveis pela persistência dos eventos;
  ...
  Agrupamento de consumidores que permite a escalabilidade horizontal para o processamento de eventos de diferentes partições em paralelo, agindo  passivamente no backpressure.
